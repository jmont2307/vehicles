    ...
    const commonQuestions = [
        {
            ...
            validate: (value) => value.trim() !== '' ? true : 'Make cannot be empty'
        },
        {
            type: 'input',
            name: 'model',
            message: 'Enter the model:',
            validate: (value) => value.trim() !== '' ? true : 'Model cannot be empty'
        },
        {
            type: 'number',
            name: 'year',
        ...
        },
        {
            ...
            validate: (value) => value.trim() !== '' ? true : 'Color cannot be empty'
        }
    ];
    let vehicle;
    if (vehicleType === 'Car') {
        ...
        vehicle = new Car(uuidv4(), answers.make, answers.model, answers.year, answers.color, answers.doors, answers.carType);
    }
        ...
        vehicle = new Truck(uuidv4(), answers.make, answers.model, answers.year, answers.color, answers.bedLength, answers.towingCapacity);
    }
    else { // Motorbike
        const answers = await inquirer.prompt([
            ...commonQuestions,
        ...
        vehicle = new Motorbike(uuidv4(), answers.make, answers.model, answers.year, answers.color, answers.engineSize, answers.bikeType);
    }
...
async function useVehicle(vehicle) {
